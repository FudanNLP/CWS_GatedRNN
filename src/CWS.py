...
class CWS(object):
    ...
    def encode(self,x,flag):
        for layer in self.layers:
            ...
            #print x.shape
        #print self.params['outputLayer_W'].shape
    ...
    def get_gradient(self,sentence,y,cmd):
        ...
        '''
        print 'my gradient:'
        print self.layers[0].g_['filter_1_W'][0][:4]
        #print self.layers[0].g_['zGate_W'][0][:4]
        #print self.layers[0].mlps[-1][0].g_['hiddenLayer_W'][0][:4]
        #print self.layers[1].g_['hiddenLayer_W'][0][:4]
        print 'gold gradient:'
        gradient = []
        for i in xrange(4):
            for layer in self.layers:
                layer.clear_layers()
            self.layers[0].params['filter_1_W'][0][i] += 0.0001
            #self.layers[0].params['zGate_W'][0][i] += 0.0001
            #self.layers[0].mlps[-1][0].params['hiddenLayer_W'][0][i] += 0.0001
            #self.layers[1].params['hiddenLayer_W'][0][i] += 0.0001
            output1 = []
            for t in xrange(len(self.id_x)):
                z = self.convert_wordId2wordVec(self.id_x[t])
                output1.append(self.encode(z,1.0))
            
            for layer in self.layers:
                layer.clear_layers()
            self.layers[0].params['filter_1_W'][0][i] -= 0.0002
            #self.layers[0].params['zGate_W'][0][i] -= 0.0002
            #self.layers[0].mlps[-1][0].params['hiddenLayer_W'][0][i] -= 0.0002
            #self.layers[1].params['hiddenLayer_W'][0][i] -= 0.0002
            output2 = []
            for t in xrange(len(self.id_x)):
                z = self.convert_wordId2wordVec(self.id_x[t])
                output2.append(self.encode(z,1.0))
            self.layers[0].params['filter_1_W'][0][i] += 0.0001
            #self.layers[0].params['zGate_W'][0][i] += 0.0001
            #self.layers[0].mlps[-1][0].params['hiddenLayer_W'][0][i] += 0.0001
            #self.layers[1].params['hiddenLayer_W'][0][i] += 0.0001
            tmp = 0.0
            for t in xrange(len(self.id_x)):
                tmp += (output1[t][y[t]] - output2[t][y[t]])
            gradient.append(tmp/0.0002)
        print gradient
        for layer in self.layers:
            layer.clear_layers()
        for t in xrange(len(self.id_x)):
