...
from GRU import GRU_LayerSetting
from RNN import RNN_LayerSetting
...
if __name__ == '__main__':
    ...
    gru_layerSetting = GRU_LayerSetting(gate_activation = Sigmoid(),activation = Tanh(),
                                        n_input_unit = windowSize,
                                        flag_use_mlps = False,
                                        wordVecLen = wordVecLen,
                                        n_rnn_layer = 2,mlp_layersizes = [75,50])
    rnn_layerSetting = RNN_LayerSetting(activation = Tanh(),
                                        n_input_unit = windowSize,
                                        flag_use_mlps = False,
                                        wordVecLen = wordVecLen,
                                        n_rnn_layer = 3,mlp_layersizes = [75,50])
    
    #layer_setting = [mlp_layerSetting]
    #layer_sizes = [wordVecLen*windowSize,150]
    layer_setting = [gru_layerSetting]
    layer_sizes = [wordVecLen*windowSize,gru_layerSetting.outputSize]
    #layer_setting[l1]
    dataSet = 'ctb6'
